package com.dyh.android.printer.buletooth.print;
...
public class BtService {
    ...
    private static final String NAME = "MyPrinterService";
    ...
    private synchronized void setState(int state) {
        ...
        // if need to show bt tabId, use the code below
        // Give the new state to the Handler so the UI Activity can update
        switch (mState) {
            case STATE_NONE:
                BluetoothPrintManager.getInstance().sendNotify(OnPrinterNotifyListener.NotifyMessage.BLUETOOTH_STATE_CHANGED_NONE);
                break;
            case STATE_LISTEN:
                BluetoothPrintManager.getInstance().sendNotify(OnPrinterNotifyListener.NotifyMessage.BLUETOOTH_STATE_CHANGED_WAITING2CONNECT);
                break;
            case STATE_CONNECTING:
                BluetoothPrintManager.getInstance().sendNotify(OnPrinterNotifyListener.NotifyMessage.BLUETOOTH_STATE_CHANGED_CONNECTING);
                break;
            case STATE_CONNECTED:
                BluetoothPrintManager.getInstance().sendNotify(OnPrinterNotifyListener.NotifyMessage.BLUETOOTH_STATE_CHANGED_CONNECTED);
                break;
            default:
                break;
        }
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    System.out.println("从打印机读取到的内容：" + String.valueOf(buffer));
                    // send data
                } catch (IOException e) {
